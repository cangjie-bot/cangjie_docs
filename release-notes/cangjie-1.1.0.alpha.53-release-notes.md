...
- 依赖模块的编译产物类型现在可以为 `executable`；
- 修复了 `macOS` 构建脚本运行时未添加运行时环境变量的问题；
- 修复了 `test` 输入目录为空时尝试读取空目录的问题；
- 修复了 workspace 下执行 `cjpm update` 时未正确生成 `lock` 条目的问题。
...
- 修复了 `Option<T>` 类型 `T` 是 `Any` 类型时查看变量结果错误的问题；
- 修复了对常规类型和泛型类型进行问号表达式求值时，执行变量查看和执行表达式时结果不一致的问题；
- 修复了 `Windows` 使用问号表达式计算报错的问题。
