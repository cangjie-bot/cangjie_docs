...
该字段可以指定脚本产物的存放路径，不指定时默认为 `build-script-cache` 文件夹。若该字段不为空，执行 `cjpm clean` 时会删除该字段所指向的文件夹，开发者需自身保证清理该目录行为的安全性。
...
- `script-dir = ""`：工作空间的脚本产物存放路径，非必需，默认为 `build-script-cache`
