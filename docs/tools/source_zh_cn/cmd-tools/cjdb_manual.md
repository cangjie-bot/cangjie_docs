...
## Python 扩展
  `cjdb` 继承了 `lldb` 使用 `Python` 脚本来增强调试能力以及自定义调试功能的能力。例如用户可以自定义命令或者扩展自定义格式化输出。详情请参考 `debug` 官方文档。

> **注意：**
>
> 由于 `lldb` 的 `Python` 扩展能力在构建时需要绑定具体的版本，所以生成的二进制也会依赖构建时使用的 `Python` 版本。用户在使用 `cjdb` 时也需要安装构建时使用的 `Python` 版本。目前 `cjdb` 在构建时使用的 `Python` 版本是  `3.11` ，也要求用户安装该 `Python` 版本。
