...
- Cangjie enums must not implement interfaces.
- Cangjie enum member functions must not use generics.
- Cangjie enum member functions must not use lambdas.
- Cangjie enums must not contain operator overloading.
- Cangjie enums must only use basic data types.
- Cangjie enums must not be extended using `extend`.
