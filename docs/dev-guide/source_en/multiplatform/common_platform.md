...
### Features Directive for Platform Specification

To streamline the development process and specify the target platform, you can use the features directive, beginning with the keyword `features`. This directive must be declared before the `package` declaration and can simplify your workflow significantly in cross-platform development.

The features directive can include the optional annotation `@NonProduct` and mandatory a set of features, where features are a series of names separated by commas. Understanding this directive can save you time and effort later on when compiling and managing platforms in your projects.

When using the features directive, note the following rules:

- The `features` directive can only appear before the `package` declaration.
- The directive should contain features set with qualified feature names separated by commas.
- Qualified feature names consist of identifiers and dots.
- An annotation `@NonProduct` can be applied on the features directive if the source set is not terminal.
- Features directives must be consistent within a source set.

> **Note:**
>
> The terminal source set means that its output will always be a binary file, whether an executable, static library, or dynamic library.

The following example defines common code and a global function `foo` for all platforms:
```cangjie
@NonProduct
features {}
package cmp
...
public platform func foo(): Unit {
    println("I am linux platform")
}
```
More details will be described in the Cross-Platform Compilation chapter.
...
#### Compilation with `cjpm`

cjpm simplifies the compilation process for common-specific platforms by handling everything for you. Here’s how it works:

1. Create a cjpm project.
    ```bash
    cjpm init --name="sample"
    ```

2. Next, create the following project structure: as introdused below:
    ```bash
    cjpm_project (package sample)
    ├── cmp
    │   ├── common
    │   │     └── common.cj
    │   ├── linux
...
3. Source sets content

    3.1 Common source set
    ```cangjie
    // common.cj
    @NonProduct
    features {}
    ...
    public common func call(): String
    ```

    > **Note:**
    >
    > `@NonProduct` is an annotation for the compiler indicating that the current source set `common` is not product. This means that no binary output will be generated by the compiler.

    3.2 Linux source set
    ```cangjie
    // linux.cj
    features {os.linux}
    package cmp

    public platform func call(): String { 
        "call from linux"
    }
    ```

    3.4 Windows source set
...
4. Defining Source Sets
    
    After defining the features, you need to specify the source sets—directories inside the `cmp` module conditionally included for build based on enabled features. Here is an example of the source set structure for a terminal app:
...
7. Run the following command in the root directory:
   ```sh
   cjpm run --no-feature-deduce --enable-features=os.linux
   ```
   ...
   cjpm run finished
   ```
For more information on developing Cangjie Multiplatform applications using cjpm, please refer to the cjpm manual.
