...
- `--target=aarch64-linux-android` defaults to API 26 for cross-compilation;
append the API level suffix explicitly for a higher version, e.g. `--target=aarch64-linux-android31` for API 31.
