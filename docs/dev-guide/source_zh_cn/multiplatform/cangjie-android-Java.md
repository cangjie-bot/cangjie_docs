...
启用在 FE 中支持非 C 语言的 Cangjie 互操作. <配置文件地址> 比如 'src/cj/config.toml' 或者 'javaCallCangjie.toml' 等toml格式配置文件均可.

需要注意的是，该选项需要搭配`--experimental --enable-interop-cjmapping=<Target Languages>`选项使用，前者支持互操作行为，后者提供对应所需的配置文件支持
Cangjie侧对目标语言的符号暴露控制以及泛型类型支持使用.
...
配置文件采用toml格式进行配置，按照包级别对于符号以及泛型实例化信息进行控制，实例参考如下：
...
[default] - 全局默认设置 - package未具体配置信息情况下采用默认设置规则

APIStrategy - 表示外部配置默认Cangjie符号对目标语言的可见性

GenericTypeStrategy - 表示外部配置默认Cangjie泛型相关API对目标语言的实例化范围

[[package]] - 表示以下内容是具体的包配置信息
    - name - package name
    - APIStrategy - 当前包中配置的符号可见性模式
        - Full
        - None
    - GenericTypeStrategy - 当前包中配置的泛型实例化模式
        - Partial
        - None
    - included_apis - 用户提供的完全限定名称将在目标语言中暴露（前提是满足公开等语法要求，否则将产生告警，
                      如果结构体中的符号需要暴露，则必须暴露该结构体；如果该结构体位于此列表中，则会发出警告）
    - excluded_apis - 与included_apis相反，其中符号将被隐藏
    - generic_object_configuration - 当前包允许进行实例化的一系列类型和符号配置

included_apis & excluded_apis 互斥，仅能存在其一.

#### Java 使用 Cangjie API 符号可见性控制

1、APIStrategy - { Full, None }, 前者对应excluded_apis, 后者对应included_apis;
2、GenericTypeStrategy - { Partial, None }, 前者对应需要对泛型进行指定类型的实例化，后者表示互操作无需使用泛型

#### Java 使用 Cangjie 泛型数据类型

1、generic_object_configuration - { name - 泛型数据类型(struct/class/interface/enum)对象名称， type_arguments - 对应实例化类型}
                                  { name - 对应实例化后上述泛型数据类型对象名称， symbols - 对应实例化数据类型内对象}

#### 符号控制&泛型实例化规格约束

- 泛型仅支持struct/class/enum/interface 的primitive类型，且类型对应Cangjie基础数据类型
- 配置文件需要用户保障配置的语法正确性，例如B.funcA为exposed，则B不允许设置为hiddened（其他场景同理）
