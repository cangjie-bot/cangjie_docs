...
1. 将仓颉接口类型作为 Java 函数的参数类型。
2. 在 Java 端实现仓颉接口，并作为仓颉接口类型的参数传递。
3. 调用仓颉接口中的默认实现。
...
1. 要求 Cangjie interface 不继承其他 interface
2. 要求 Cangjie interface 成员函数中不使用泛型
3. 要求 Cangjie interface 成员函数非 static
4. 要求 Cangjie interface 中仅使用基础的数据类型
5. 要求 Cangjie 不适应 extend 对 interaface 进行拓展
6. 不支持 option
