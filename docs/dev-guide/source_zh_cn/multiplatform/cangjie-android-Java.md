...
支持Java使用Cangjie Class, 规格如下：
- 允许Java类继承Cangjie的 public open Class。
- 允许访问父类中的非private方法(包括public、protected方法)。
- 允许重写父类中的非静态非private open方法(包括public、protected方法)。
- 支持基础数据类型作为函数参数类型。
...
- 不支持使用仓颉泛型Class
- 不支持访问继承Cangjie class的普通成员变量、静态成员变量、静态成员方法、prop成员。
- 不支持跨包引用