...
- 要求 Cangjie enum 无 interface 实现
- 要求 Cangjie enum 成员函数中不使用泛型
- 要求 Cangjie enum 成员函数中不使用 Lamda
- 要求 Cangjie enum 中不包含操作符重载
- 要求 Cangjie enum 中仅使用基础的数据类型
- 要求 Cangjie 不适应 extend 对 enum 进行拓展
- 不支持 option
