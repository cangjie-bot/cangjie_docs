...
- 仅允许使用基础数据类型：
    ...
    - Bool 类型
    - Unit 类型
- 已支持的互操作特性不得使用interface作为函数返回值类型
...
#import <dlfcn.h>
#import <stdlib.h>
static int64_t (*CJImpl_ObjC_UNNAMED_Vector_init__ii)(int32_t,int32_t) = NULL;
static void (*CJImpl_ObjC_UNNAMED_Vector_deleteCJObject)(int64_t) = NULL;
static int32_t (*CJImpl_ObjC_UNNAMED_Vector_x_get)(int64_t) = NULL;
static int32_t (*CJImpl_ObjC_UNNAMED_Vector_y_get)(int64_t) = NULL;
static int64_t (*CJImpl_ObjC_UNNAMED_Vector_dot_RN7UNNAMED6VectorE)(int64_t,int64_t) = NULL;
static void (*CJImpl_ObjC_UNNAMED_Vector_processPrimitive___idb)(int32_t,double,BOOL) = NULL;
static void* CJWorldDLHandle = NULL;
static struct RuntimeParam defaultCJRuntimeParams = {0};
@implementation Vector
- (id)init:(int32_t)x :(int32_t)y {
    if (self = [super init]) {
        self.$registryId = CJImpl_ObjC_UNNAMED_Vector_init__ii(x, y);
    }
    return self;
}
- (id)initWithRegistryId:(int64_t)registryId {
...
+ (void)initialize {
    if (self == [Vector class]) {
        ...
        if (LoadCJLibraryWithInit("libUNNAMED.dylib") != E_OK) {
            NSLog(@"ERROR: Failed to init cjworld library ");
            exit(1);
        }
        if ((CJWorldDLHandle = dlopen("libUNNAMED.dylib", RTLD_LAZY)) == NULL) {
            NSLog(@"ERROR: Failed to open cjworld library ");
            NSLog(@"%s", dlerror());
            exit(1);
        }
        if ((CJImpl_ObjC_UNNAMED_Vector_init__ii = dlsym(CJWorldDLHandle, "CJImpl_ObjC_UNNAMED_Vector_init__ii")) == NULL) {
            NSLog(@"ERROR: Failed to find CJImpl_ObjC_UNNAMED_Vector_init__ii symbol in cjworld");
            exit(1);
        }
        if ((CJImpl_ObjC_UNNAMED_Vector_deleteCJObject = dlsym(CJWorldDLHandle, "CJImpl_ObjC_UNNAMED_Vector_deleteCJObject")) == NULL) {
            NSLog(@"ERROR: Failed to find CJImpl_ObjC_UNNAMED_Vector_deleteCJObject symbol in cjworld");
            exit(1);
        }
        if ((CJImpl_ObjC_UNNAMED_Vector_x_get = dlsym(CJWorldDLHandle, "CJImpl_ObjC_UNNAMED_Vector_x_get")) == NULL) {
            NSLog(@"ERROR: Failed to find CJImpl_ObjC_UNNAMED_Vector_x_get symbol in cjworld");
            exit(1);
        }
        if ((CJImpl_ObjC_UNNAMED_Vector_y_get = dlsym(CJWorldDLHandle, "CJImpl_ObjC_UNNAMED_Vector_y_get")) == NULL) {
            NSLog(@"ERROR: Failed to find CJImpl_ObjC_UNNAMED_Vector_y_get symbol in cjworld");
            exit(1);
        }
        if ((CJImpl_ObjC_UNNAMED_Vector_dot_RN7UNNAMED6VectorE = dlsym(CJWorldDLHandle, "CJImpl_ObjC_UNNAMED_Vector_dot_RN7UNNAMED6VectorE")) == NULL) {
            NSLog(@"ERROR: Failed to find CJImpl_ObjC_UNNAMED_Vector_dot_RN7UNNAMED6VectorE symbol in cjworld");
            exit(1);
        }
        if ((CJImpl_ObjC_UNNAMED_Vector_processPrimitive___idb = dlsym(CJWorldDLHandle, "CJImpl_ObjC_UNNAMED_Vector_processPrimitive___idb")) == NULL) {
            NSLog(@"ERROR: Failed to find CJImpl_ObjC_UNNAMED_Vector_processPrimitive___idb symbol in cjworld");
            exit(1);
        }
    }
}
- (int32_t)x {
    return CJImpl_ObjC_UNNAMED_Vector_x_get(self.$registryId);
}
- (int32_t)y {
    return CJImpl_ObjC_UNNAMED_Vector_y_get(self.$registryId);
}
- (int64_t)dot:(Vector*)v {
    return CJImpl_ObjC_UNNAMED_Vector_dot_RN7UNNAMED6VectorE(self.$registryId, v.$registryId);
}
+ (void)processPrimitive:(int32_t)intArg :(double)floatArg :(BOOL)boolArg {
     CJImpl_ObjC_UNNAMED_Vector_processPrimitive___idb(intArg, floatArg, boolArg);
}
- (void)deleteCJObject {
     CJImpl_ObjC_UNNAMED_Vector_deleteCJObject(self.$registryId);
}
- (void)dealloc {
    [self deleteCJObject];
}
...
为实现 Cangjie 与 Objective-C 的互操作，需将 Cangjie 的非 open 类映射为 ObjC 的类。映射后，用户可：

- 在 ObjC 中调用 Cangjie 侧 public class 的 public 实例方法、静态方法
...
static int64_t (*CJImpl_ObjC_UNNAMED_Vector_init__ii)(int32_t,int32_t) = NULL;
static void (*CJImpl_ObjC_UNNAMED_Vector_deleteCJObject)(int64_t) = NULL;
static int64_t (*CJImpl_ObjC_UNNAMED_Vector_dot_CN7UNNAMED6VectorE)(int64_t,int64_t) = NULL;
static void (*CJImpl_ObjC_UNNAMED_Vector_processPrimitive___idb)(int32_t,double,BOOL) = NULL;
static void* CJWorldDLHandle = NULL;
static struct RuntimeParam defaultCJRuntimeParams = {0};
@implementation Vector
...
+ (void)initialize {
    if (self == [Vector class]) {
        ...
        if ((CJImpl_ObjC_UNNAMED_Vector_dot_CN7UNNAMED6VectorE = dlsym(CJWorldDLHandle, "CJImpl_ObjC_UNNAMED_Vector_dot_CN7UNNAMED6VectorE")) == NULL) {
            NSLog(@"ERROR: Failed to find CJImpl_ObjC_UNNAMED_Vector_dot_CN7UNNAMED6VectorE symbol in cjworld");
            exit(1);
        }
        if ((CJImpl_ObjC_UNNAMED_Vector_processPrimitive___idb = dlsym(CJWorldDLHandle, "CJImpl_ObjC_UNNAMED_Vector_processPrimitive___idb")) == NULL) {
            NSLog(@"ERROR: Failed to find CJImpl_ObjC_UNNAMED_Vector_processPrimitive___idb symbol in cjworld");
...
- (int64_t)dot:(Vector*)v {
    return CJImpl_ObjC_UNNAMED_Vector_dot_CN7UNNAMED6VectorE(self.$registryId, v.$registryId);
}
+ (void)processPrimitive:(int32_t)intArg :(double)floatArg :(BOOL)boolArg {
     CJImpl_ObjC_UNNAMED_Vector_processPrimitive___idb(intArg, floatArg, boolArg);
}
...
- 仅支持 public 成员函数映射，其它成员不会报错，不会映射到ObjC端
- 不支持泛型 class
- 不支持泛型成员函数
- 不支持函数重载
- 成员函数参数和返回值允许使用基础数据类型（数值类型、Bool 类型和 Unit 类型）和本包定义的非 open public class类型
