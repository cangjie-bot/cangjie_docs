...
- platform 全局函数的函数签名必须与同包的 common 全局函数匹配，即参数类型和返回值类型必须一致，并需要同时满足以下规则：
    ...
    - 函数默认参数只能在 common 侧或 platform 侧其中一侧拥有，不能在两侧同时拥有默认值。当 common 全局函数包含默认值时，platform 全局函数的相应位置必须为同参数名的命名参数，且不支持默认值。
...
**重要限制：** common sealed class 的所有直接子类型必须定义在同一个 common 包中。
...
- 不可以在 abstract platform class（抽象具体类）中额外添加抽象成员。
