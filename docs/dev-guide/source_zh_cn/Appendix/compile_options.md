...
`gnu` 的 `ld` 链接 `mingw` 的 `PE` 目标时，`-la` 会链接 `liba.dll`，`liba.a`，`liba.lib`，但是 `llvm` 的 `lld` 链接 `mingw` 的 `PE` 目标时，`-la` 只会链接 `liba.dll`，`liba.a`。

在 `Windows` 环境中，使用 `llvm` 的 `lld` 链接器需要链接 `OpenSSL` 库文件，编译时出现如下报错：
...
报错原因是缺少 `libssl.lib`，`libcrypto.lib` 链接库，需要手动添加的命令如下：
