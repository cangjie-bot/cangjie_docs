...
> 同时写 `--static-std` 与 `--dy-std` 时，**后出现的生效**。  
> 若最终产物里会引入仓颉动态库（`--output-type=dylib`），则对应的可执行单元**必须**显式加 `--dy-std`，否则运行期可能出现多份标准库副本。

3. `--static-std` 在`ohos` 平台中无效，该平台只支持动态链接。各个平台支持能力如下：

这里是**纯 Markdown 格式**的居中对齐表格，可直接在支持标准 Markdown 的编辑器（如 Typora、GitHub、GitLab）中使用：

| 生成产物平台 | `--static-std` | `--dy-std` |
|:-------------------:|:--------------:|:----------:|
| Linux         |       ✔        |     ✔      |
| macOS        |       ✔        |     ✔      |
| Windows       |       ✔        |     ✔      |
...
### 格式原理
Markdown 表格的对齐规则由表头下方的分隔线控制：
- `:---:` = 居中对齐
- `:---`  = 左对齐
- `---:`  = 右对齐

需要我帮你把这个表格**转换成适合技术文档的简洁无框样式**吗？
