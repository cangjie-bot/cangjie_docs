...
支持的目标平台的 ABI 有：`ohos`、`gnu`、`musl`、`simulator`、`android`。
...
仓颉交叉编译至目标平台 `aarch64-linux-android31`，条件变量设置如下面代码所示，至于交叉编译至其他平台，条件编译的设置请参考 [目标平台和条件编译映射表](#目标平台和条件编译映射表)
...
仓颉交叉编译支持的目标平台和仓颉提供的内置条件变量 `os`、 `arch`、 `env` 存在映射关系，将映射关系整理成如下表格：
...
| aarch64-apple-ios-simulator   | "aarch64" | "iOS"     | "simulator" |
| aarch64-apple-darwin          | "aarch64" | "macOS"   |             |
| aarch64-linux-ohos            | "aarch64" | "Linux"   | "ohos"      |
| aarch64-apple-ios11           | "aarch64" | "iOS"     |             |
| aarch64-apple-ios11-simulator | "aarch64" | "iOS"     | "simulator" |

<sup>[android target]</sup> x86_64-linux-android 后缀的数字用于指定目标 Android API Level。未指定数字时，默认 API Level 为 31；指定数字（如 x86_64-linux-android33）表示目标为该 API Level，指定的数字应大于等于31。