...
当前仓颉支持从 ARM 架构环境交叉编译生成 iOS 模拟器静态库，既适配 ARM 架构模拟器，也可编译至 x86_64 架构模拟器（该架构产物需依赖 Xcode 的 Rosetta 运行）。若需在 iOS 模拟器上运行，需指定以下选项：

- `--target`
  - `--target=aarch64-apple-ios-simulator` 适用于arch64目标架构
