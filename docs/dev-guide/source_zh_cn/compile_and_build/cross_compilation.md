...
支持交叉编译至 `iOS` 的仓颉安装包 `cangjie-sdk-mac-aarch64-ios.x.y.z.tar.gz`以及 `cangjie-sdk-mac-x86_64-ios.x.y.z.tar.gz`。

若需要在 `macOS aarch64` 平台交叉编译至 `iOS`，可根据宿主机架构选择对应的仓颉软件包。宿主机为 Apple Silicon(M1/M2…)可以下载安装 `cangjie-sdk-mac-aarch64-ios.x.y.z.tar.gz` 仓颉软件包。宿主机为 Intel x86_64可以下载安装 `cangjie-sdk-mac-x86_64-ios.x.y.z.tar.gz` 软件包。仓颉运行时以及标准库默认支持运行于 `iOS 11` 及以上系统（对于例外场景，请见《仓颉编程语言库 API》手册）。
...
- `--target=aarch64-apple-ios-simulator` 或 `--target=x86_64-apple-ios-simulator`指定目标平台 `ios-simulator` 进行交叉编译
- `--output-type=staticlib` 指定输出文件的类型为静态库
