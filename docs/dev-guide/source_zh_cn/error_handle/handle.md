...
- 仓颉支持创建异常链（仅 `Exception`），如表达式 `let fatherException = FatherException("this is message", causeException)`，causeException 也是一个异常，是 fatherException 的触发原因。在打印异常的调用栈时，会递归打印触发原因。
